...
# import pandas as pd
# import numpy as np
# from model_utils import DeepMLPClassifier, mean_pooling
...
#         print(f"[INFO] Обработано {min(i+batch_size, len(texts))}/{len(texts)} текстов")
#
# sentence_embeddings = torch.cat(all_embeddings, dim=0)
# print(f"[INFO] Эмбеддинги сформированы. Размер: {sentence_embeddings.shape}")
#
# # --- Dataset ---
...
# model_mlp = DeepMLPClassifier(input_dim=sentence_embeddings.shape[1]).to(device)
# optimizer = torch.optim.Adam(model_mlp.parameters(), lr=1e-3)
#
# # Подсчёт количества примеров каждого класса
# label_counts = Counter(df["majority_vote"].map(label_map))
# total = sum(label_counts.values())
#
# # Веса: чем меньше класс — тем больше вес
